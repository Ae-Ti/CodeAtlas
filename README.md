...
측정은 제출물 경로 그대로입니다 — `seed_dump.sql` 복원 + 임베딩 backfill 후
`eval_set_tool.py resolve` → `eval_retrieval.py --multi-gold`. 두 팀원의 독립 환경에서
동일 수치가 재현됐습니다(2026-08-14).

MRR 은 순위를 어디서 자르느냐에 따라 달라지므로 컷을 함께 적습니다 — 위 값은
`--k` 기본값 10 기준입니다. 같은 정답셋에서 `--k 5` 는 0.428, 후보를 자르지 않으면
(단락당 최대 33블록) 0.464 입니다. Top-1/3/5 는 컷과 무관하게 동일합니다.
